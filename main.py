...
# Доступные подарки (согласно GiftEffects.luau)
...
def webhook():
    ...
    # Обработка нажатий Inline-кнопок (выбор подарка)
    if "callback_query" in update:
        ...
        if data.startswith("gift_"):
            ...
            if roblox_nick and send_gift(roblox_nick, gift_type):
                send_telegram_msg(chat_id, f"✅ Подарок <b>{gift_type}</b> отправлен игроку <b>{roblox_nick}</b>!")
            else:
                send_telegram_msg(chat_id, "❌ Ошибка при отправке подарка.")
    ...
    if "message" in update:
        ...
        # Если не зарегистрирован — берем сообщение как ник
        if not roblox_nick:
            register_user(chat_id, text)
            send_telegram_msg(chat_id, f"✅ Твой Roblox ник успешно сохранен: <b>{text}</b>", reply_markup=get_main_keyboard())
            return jsonify({"status": "ok"}), 200
        ...
        if text == "👥 Список игроков":
            ...
            send_telegram_msg(chat_id, f"Выбери подарок для отправки себе (<b>{roblox_nick}</b>):", reply_markup=keyboard)
            ...
            if res.status_code == 200:
                ...
                send_telegram_msg(chat_id, "❌ Не удалось загрузить логи.")

        elif text == "👤 Профиль":
            send_telegram_msg(chat_id, f"👤 <b>Твой профиль:</b>\n\n"
                                       f"🆔 Telegram ID: <code>{chat_id}</code>\n"
                                       f"🎮 Roblox Nick: <b>{roblox_nick}</b>")
